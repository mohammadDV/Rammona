<template>
  <BulmaCard class="solution-list p-gap">
    <h2 class="mb-gap">{{ $t("apps.solution") }}</h2>
    <BulmaTable
      :length="websiteStore.solutions.length"
      :loading="fetchLoading"
      :headers="['icon', 'title', 'content', 'action']"
    >
      <tr v-for="(item, index) in websiteStore.solutions" :key="index">
...
const { fetchLoading } = useStoreFetch(websiteStore.getSolutions);
...
function handleDelete(id: number) {
  websiteStore.deleteSolution(id).then(() => {
    resetDialog();
  });
}
</script>
